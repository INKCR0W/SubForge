...
use app_core::{CoreError, PluginInstallService, SourceService, SourceWithConfig, StaticFetcher};
use app_secrets::SecretStore;
use app_storage::{
    Database, NodeCacheRepository, PluginRepository, ProfileRepository, SettingsRepository,
    SourceRepository, StorageError,
};
use axum::body::Body;
...
async fn refresh_source_handler(
    ...
    let url = source
        ...
        .ok_or_else(|| config_error_response("来源配置缺少 url 字段"))?
        .to_string();
    ...
    let fetcher = StaticFetcher::new(state.database.as_ref()).map_err(core_error_to_response)?;
    let result = fetcher
        .fetch_and_cache(&id, &url, user_agent.as_deref())
        .await;
    ...
    match result {
        Ok(nodes) => {
            emit_event(
                &state,
                "refresh:complete",
                format!("来源刷新成功：{id}，节点 {} 条", nodes.len()),
                Some(id.clone()),
            );
            Ok((
                StatusCode::OK,
        ...
        let found = connection
            .query_row(
                "SELECT 1
                 ...
                 LIMIT 1",
                rusqlite::params![profile_id, token, now],
                |_| Ok(()),
            )
            .optional()?
            .is_some();
